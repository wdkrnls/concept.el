...
By default the length of a section is the number of characters which are needed to represent it."
  ...
  (when (null length-b)
    (setq length-b 0))
  (or (string< (downcase a) (downcase b))
      (and (string= a b)
           (< length-a length-b))))
...
this."
  ...
  (beginning-of-line)
  (open-line 1)
  (when (not (concept-current-line-blank-p))
    (backward-char))
  (insert "| ")
...
(defun concept-repeat-current-block ()
  ...
  (interactive)
  (let ((phrase (thing-at-point 'sexp t)))
    (save-excursion
...
(defun concept-kill-dwim ()
  ...
  (cond ((concept-on-focus-line)
         ...
         (kill-line))
        ((concept-on-relationship-line)
         (kill-region (line-beginning-position) (save-excursion (concept-goto-next-relationship-boundary) (point)))
         (end-of-line))
        ((concept-on-data-concept-line)
         (kill-whole-line)
         (end-of-line))
  ...
  (cond ((concept-on-focus-line)
        ...
        ((concept-on-relationship-line)
         (let ((rel (concept-current-relationship))
           ...
           (cond ((and (string= "take" rel)
                       (not (member "give" rels)))
                  (concept-insert-relationship-group-at-end* "give"))
                 ((and (string= "give" rel)
                       (not (member "make" rels)))
                  (concept-insert-relationship-group-at-end* "make"))
                 ((and (string= "give" rel)
                       (not (member "take" rels)))
                  (concept-insert-relationship-group-at-end* "take"))
                 ((and (string= "have" rel)
                       (not (member "hold" rels)))
                  (concept-insert-relationship-group-at-end* "hold"))
                 ((and (string= "include" rel)
                       (not (member "have" rels)))
                  (concept-insert-relationship-group-at-end* "have"))
                 (t
                  (concept-insert-relationship-group-at-end* "include")))))
        ((concept-on-exposition-line)
         (let ((key (concept-current-attribute))
               (keys (concept-resource-block-keys)))
           (cond ((and (string= "question" key)
                       (not (member "answer" keys)))
                  (concept-insert-keyword-block-after* "answer"))
                 ((and (member key (list "line" "search-phrase"))
                       (not (or (member "file-name" keys)
                                (member "file" keys)))
                 ...
                 ((and (string= "shell-command" key)
                  ...
                  (concept-insert-keyword-block-after* "prompt"))
        ...
        ((concept-on-attribute-line)
         (let ((key (concept-current-attribute)))
           (cond ((string= key "question")
                  (concept-insert-keyword-block-at-end* "answer"))
                 (t 
...
(defun concept-add-dwim (arg)
  "Add what I mean to add.
This largely operates above the current line unless the prefix arg is
supplied, in which case it goes below."
  (interactive "p")
  (beginning-of-line)
  (let ((p (thing-at-point 'symbol t)))
    (cond ((concept-on-first-line-p)
           (beginning-of-line)
           (open-line 1)
           (insert "~ "))
          ((concept-on-resource-line)
...
(defun concept-insert-include-group ()
  "Insert an include group.
Such lines go inside a relationship block."
  (when (concept-in-relationship-block)
    (concept-goto-next-relationship-boundary)
    (backward-char)
...
(defun concept-alphabetic-sort-dwim (arg)
  ...
  (interactive "p")
  (if (or (null arg) (/= 4 arg))
      (let ((concept-map-bubble-sort-maximum-iterations
             (if (null arg)
                 concept-map-bubble-sort-maximum-iterations
               (* 1000 arg))))
        (cond ((concept-on-data-concept-line)
...
can think of this as analogous to how C-. and M-. work."
  ...
  (cond ((and (concept-on-data-concept-line)
              (save-excursion
                ...
                (or (concept-on-focus-line)
                    (concept-on-resource-line))))
         (let ((next-focus (concept-next-focus)))
           (end-of-line)
           (insert "\n~ " next-focus)))
        ((concept-on-relationship-line)
...
modifying `mailcap-user-mime-data'."
  ...
  (cond ((and (concept-on-exposition-line)
        ...
        ((and (concept-on-exposition-line)
              (string= "emacs-value" (concept-exposition-parent-key)))
         (apropos-value (concept-current-exposition)))
        ((and (concept-on-exposition-line)
              (string= "emacs-option" (concept-exposition-parent-key)))
         (apropos-user-option (concept-current-exposition)))
        ((and (concept-on-exposition-line)
