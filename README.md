...
For what is functional, the few network analysis tools we have do seem to work. However, they are incomplete in that we really need to develop and integrate the tabulated-list mode query views for command variants than should report more than one concept ranked by a depth or a count. 
...
### Incremental updating of conceptual relationship networks

Our Emacs-based network analysis interface is currently pretty fresh, largely incomplete, and not yet as scalable as it needs to be. To be scalable `concept.el` needs a network updating system which minimizes the amount of computation that must be done to keep the network hash table in-sync. And the highest level there the question it most solve is:

> when is it more productive to incrementally update the graph rather than to recompute it from scratch?

We believe we are on the right track towards implementing an incremental network updating system. However, it will be naturally much more complicated than the full-graph build system we have right now. We have realized that the only feasible way to do it is to add an intermediate step for constructing the network: we must create an edge counting facility which tracks the counts for the parent-child conceptual relationships in the graph. We already have prelimary functions for the basic operations. What is left is to connect them to snapshot diff analysis system.

Our `concept-map--buffer-changed-line-numbers` procedure scans the unified `diff` command output comparing the latest snapshot with the current buffer. Right now it returns negative numbers when there were deletions in the snapshot buffer but no corresponding lines in the edited buffer.  This must be changed. It must give us all the line numbers that changed for both the snapshot buffer and the concept map buffer. Then we must consolidate those numbers to point to all the modified/deleted relationship blocks. Given the high information density enabled by the terse relationship block syntax, it's probably best to forgo extensive case analysis. Instead, the far simpler solution is to fully decrement all the edge counts created by the edited relationship blocks, and then count up all the new versions of the relationship blocks from scratch. This might not be the most efficient approach, but it will be far more efficient than reprocessing the whole buffer, while also being far simpler to implement. Checking on all the ways the edges change when focus lines, relationship groups, or data concepts are modified would be exhausting and error prone with an almost surely brittle result.

Breaking down our plan into actionable steps, we need a way to reliably point to the focus concept of each affected relationship block and perform the edge recounts. Defining a function which reports the line-region of each concept should pair well with some  overlap arithment to will help us write a function which returns just the line numbers of the focus lines of the  affected relationship blocks. We can then write a function which removes the edge counts for all the affected relationship blocks in the snapshot buffer and add back the edge counts for the affected relationship blocks in the edited buffer. We can probably do this in a way we can reuse for making the edge counts for the whole buffer.

We are still left with our main question. When is incremental processing more efficient than a complete reprocessing? Honestly, think through it I'm hoping the answer is never. We should only have to do that counting when the buffer first opens.
